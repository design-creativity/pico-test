...
class ICM20948PlotterWindows:
    ...
    def find_pico_port(self):
        ...
        for port in ports:
            ...
            if any(keyword in port.description.lower() for keyword in 
                   ['pico', 'raspberry pi', 'rp2040', 'micropython']):
                pico_ports.append(port.device)
                print(f"   Raspberry Pi Pico found: {port.device}")
            
            elif 'usb serial device' in port.description.lower():
                usb_serial_ports.append(port.device)
                print(f"   USB Serial Device found: {port.device} (likely Pico)")
        
        if pico_ports:
            print(f"Using detected Pico port: {pico_ports[0]}")
            return pico_ports[0]
        
        if usb_serial_ports:
            print(f"Using USB Serial Device (likely Pico): {usb_serial_ports[0]}")
            return usb_serial_ports[0]
        
        regular_ports = []
        for port in ports:
            if not any(skip_keyword in port.description.lower() for skip_keyword in 
                      ['intel', 'management', 'amt', 'sol', 'system']):
                regular_ports.append(port.device)
                print(f"   Trying regular port: {port.device}")
        
        if regular_ports:
            return regular_ports[0]
    ...
    def setup_serial(self):
        ...
        if not serial_port:
            ...
            print("Manual steps:")
            print("   1. Check Device Manager for COM ports")
            print("   2. Make sure Pico is connected via USB")
            print("   3. Close Thonny if it's open")
            print("   4. Update SERIAL_PORT in script manually")
            exit(1)
            ...
            for i in range(5):
                if self.serial_conn.in_waiting > 0:
                    ...
                    if line:
                        ...
                        if 'Reading' in line or 'Accel' in line:
                            data_found = True
                        break
                time.sleep(1)
            
            if not data_found:
                print("   No IMU data detected on this port")
                print("   Make sure main.py is running on Pico (press Ctrl+D in Thonny)")
